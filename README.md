...
This app uses [androidtvremote2](https://github.com/tronikos/androidtvremote2) and a custom Python script to create a volume control menu bar widget for Android TVs. This has only been tested on a *TCL Percee TV* running Android 8. If you have a different model of Android television, you may need to modify `scripts/tvvolume.py`.
...
```sh
	...
	"$PYTHON_EXECUTABLE" "tvvolume.py" > /dev/null 2>&1 &
```
6. Save the app as "TVVolumeApp" and close Automator.
7. Create your own app icon or use the one provided; name it `AppIcon.icns` and place it in `TVVolumeApp.app/Contents/Resources/`.
8. Modify the strings in your "TVVolumeApp" app bundle:
	```sh
